...
class Bookmarks:
    ...
    def extract_bookmarks_per(self):
        ...
        for like in self.data_file["orderedItems"]:
            ...
            bookmarks_per_user.update({f"@{user}@{server}": bookmarks_per_user[f"@{user}@{server}"] + 1})
        ...
        self.bookmarks_per_server = DataFrame(
            data=Counter(self.stats["bookmarks_per_server"]).most_common(len(self.stats["bookmarks_per_server"])),
            columns=["Server", "Count"]
        )
        self.bookmarks_per_user = DataFrame(
            data=Counter(self.stats["bookmarks_per_user"]).most_common(len(self.stats["bookmarks_per_user"])),
            columns=["User", "Count"]
        )

    def extract_most_least(self):
        self.stats["most_bookmarked_server"] = self.bookmarks_per_server.iloc[self.bookmarks_per_server["Count"].idxmax()]["Server"]
        self.stats["least_bookmarked_server"] = self.bookmarks_per_server.iloc[self.bookmarks_per_server["Count"].idxmin()]["Server"]
        self.stats["most_bookmarked_user"] = self.bookmarks_per_user.iloc[self.bookmarks_per_user["Count"].idxmax()]["User"]
        self.stats["least_bookmarked_user"] = self.bookmarks_per_user.iloc[self.bookmarks_per_user["Count"].idxmin()]["User"]
